...
import { DataBaseType, DateFormatType, SqlType } from '@const/ConstDefine';
...
export default class SqlUtils {
    ...
    * @returns 
    */
   static getOracleDateType = (dateFormatType) => {
    ...
    * @returns 
    */
   static partseSqlDate = (momentObject, dateFormatType) => {
  ...
  static buildWhereClause = (queryFields, formValues) => {
      ...
      for (let property in formValues) {
         if (formValues[property]) {
               ...
               if (moment.isMoment(formValues[property])) {
                  formValues[property] = this.partseSqlDate(values[property], DateFormatType.Date);
               }
               if (Array.isArray(formValues[property])) {
                  // 如果第一个栏位不是moment的话，则说明不是时间数组，则跳过
                  if (!moment.isMoment(formValues[property][0])) {
                     continue;
                  }
                  // 当前处理为0点0分0秒到23点59分59秒。即如果from 4号 to 4号。就是4号零点到4号23点59分59秒。
                  let fromDate = formValues[property][0].hour(0).minute(0).second(0);
                  let toDate = formValues[property][1].hour(23).minute(59).second(59);
                  formValues[property] = [fromDate, toDate]
               }
         }
      }     
